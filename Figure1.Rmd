...
fit <- nls_multstart(dr5~lactin2_1995(temp = temp, a,b,tmax,delta_t),
                                                     data = d,
                                                     iter = 500,
...
ggplot(new_data, aes(temp, .fitted)) +
  geom_line(aes(temp, .fitted), preds, col = 'grey40', size = 2) +
  theme_cowplot()+
  ylab("Development rate")+
  xlab("Temperature")+
  ylim(0, 1.1)+
  xlim(2, 40)+
  geom_segment(x = 10, y = 0, xend = 27.4, yend = 1.1, size = 2, linetype = 2)+
   
  #annotate("text", x = 9, y = -0.08, label = expression("T"[0]))+
  theme(axis.text = element_text(size = 16))+
  #theme(axis.text.x= c("Tmin", "T0", "Topt", "Tmax"))+
  scale_x_continuous(breaks = c(7, 10, 27, 32), labels = c(expression("T"[min]), expression("T"[0]), expression("T"[opt]), expression("T"[max])))+
